...
$results = mysql_query("select `id`, `title` from `song` where `current` = 0 order by `title` asc");
if (! $results) die("Database query failed.");
while ($result = mysql_fetch_array($results)) echo "<tr><td id=\"row_$result[0]\" class=repertoire_row>$result[1]</td></tr>";
echo "</table>";
?></div>
...
<div class="modal-body" id=confirm_delete_song_body>Are you sure you want to delete this song?  All links associated with it will be removed, and any files associated with it stored on the Glee Club server will be deleted.</div>
