...
if (! mysql_query("insert ignore into `attends` (`memberID`, `eventNo`, `shouldAttend`) select '$USER', `eventNo`, `defaultAttend` from `event` where `semester` = '$SEMESTER' and `choir` = '$CHOIR' and (`type` != 'sectional' or `section` = $sect or `section` = 0)")) die("Error setting attendance relationships: " . mysql_error());
echo "OK";
?>
