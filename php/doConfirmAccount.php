...
if (query("select * from `activeSemester` where `member` = ? and `semester` = ? and `choir` = ?", [$USER, $SEMESTER, $CHOIR], QCOUNT) != 0) err("Already confirmed");
$loc = $_POST['location'];
$reg = $_POST['registration'];
$sect = $_POST['section'];
if ($reg != "class" && $reg != "club") err("Invalid registration \"$reg\"");
