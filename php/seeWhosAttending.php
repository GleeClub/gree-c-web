...
$sql = 'SELECT prefName,firstName,lastName,section,`attends`.`confirmed` as confirmed,section,shouldAttend FROM `attends`,`member` WHERE eventNo='.$eventNo.' AND memberID=email and member.confirmed=\'1\' order by confirmed DESC,section ASC,lastName ASC,firstName ASC';
...
while($row = mysql_fetch_array($results, MYSQL_ASSOC)){
	...
	if($row['shouldAttend'] == '1'){
		$shouldHtml = $shouldHtml.'
			<tr>
				<td>' . ($prefName == '' ? $firstName : $prefName) . '  ' .$lastName . '</td>
