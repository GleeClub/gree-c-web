...
if (! hasPermission("edit-tie")) err('DENIED');
if (! isset($_POST['action'])) err('MISSING_ARG');
if ($action == 'return')
{
	if(! isset($_POST['member'])) err('MISSING_ARG');
	if (query("select * from `tieBorrow` where `member` = ? and `dateIn` is null", [$member], QCOUNT) == 0) err("$member does not have a tie out");
	query("update `tieBorrow` set `dateIn` = curdate() where `member` = ? and `dateIn` is null", [$member]);
	echo "OK";
}
else if ($action == 'checkout')
{
	if (! isset($_POST['tie']) || ! isset($_POST['member'])) err('MISSING_ARG');
	if (query("select * from `tie` where `id` = ?", [$tie], QCOUNT) == 0) err("Tie $tie does not exist");
	$ties = query("select * from `tieBorrow` where `member` = ? and `dateIn` is null", [$member], QONE);
	if ($ties)
	{
		$oldtie = $ties["tie"];
		err(memberName($member) . " already has tie $oldtie out");
	}
	$result = query("select * from `tieBorrow` where `tie` = ? and `dateIn` is null", [$tie], QONE);
	if ($result) err("Tie $tie is already checked out to " . memberName($result["member"]));
	query("insert into `tieBorrow` (`member`, `tie`, `dateOut`) values (?, ?, curdate())", [$member, $tie]);
	echo "OK";
}
else if ($action == 'add')
{
	if (! isset($_POST['tie'])) err('MISSING_ARG');
	if (! preg_match('/^\d+$/', $tie)) err("Invalid tie number \"$tie\"");
	if (query("select * from `tie` where `id` = ?", [$tie], QCOUNT) > 0) err("Tie $tie already exists");
...
{
	if (! isset($_POST['newid']) || ! isset($_POST['status']) || ! isset($_POST['comments']) || ! isset($_POST['tie'])) err('MISSING_ARG');
	query("update `tie` set `id` = ?, `status` = ', `comments` = ? where `id` = ?", [$newid, $status, $comments, $tie]);
	echo "OK";
}
else if ($action == 'histdel')
{
	if (! isset($_POST['id'])) err('MISSING_ARG');
